...
# 默认参数有个大坑, 含默认参数的函数被调用多次后，如果默认参数被改变，后续默认参数的默认值就是改变后的值，不是最初定义的值，所以默认参数需要为不可变值，如None，String, 数字等
def power(x, n=2):
    s = 1
    while n > 0:
        n -= 1
...
# 在函数调用的时候，Python解释器自动按照参数位置和参数名把对应的参数传进去。最神奇的是通过一个tuple和dict，你也可以调用该函数，所以，对于任意函数，都可以通过类似func(*args, **kw)的形式调用它，无论它的参数是如何定义的。
...
print(chr(0x3d))                 # 返回值为=。chr(n)返回数字n对应的ascii码字符，数字n in range(0, 255)，参数如果超出这个范围会报错
print(hex(ord('尘')))            # 0x5c18。ord(c)返回对应字符的 ASCII 数值，或者 Unicode 数值，是10进制整数。hex()函数将数字转换成16进制


'''
Python查找一个变量时会按照“局部作用域”、“嵌套作用域”、“全局作用域”和“内置作用域”的顺序进行搜索，前三者我们在上面的代码中已经看到了，所谓的“内置作用域”就是Python内置的那些隐含标识符min、len等都属于内置作用域）
在实际开发中，我们应该尽量减少对全局变量的使用，因为全局变量的作用域和影响过于广泛，可能会发生意料之外的修改和使用，除此之外全局变量比局部变量拥有更长的生命周期，可能导致对象占用的内存长时间无法被垃圾回收。事实上，减少对全局变量的使用，也是降低代码之间耦合度的一个重要举措，同时也是对迪米特法则的践行。减少全局变量的使用就意味着我们应该尽量让变量的作用域在函数的内部，但是如果我们希望将一个局部变量的生命周期延长，使其在函数调用结束后依然可以访问，这时候就需要使用闭包，装饰和匿名函数是不同的概念
...
def foo6():
    ...
    def bar():
        b = 500
        print(a)
        # a = 500    # 在内部函数中定义与外部函数同名的局部变量，变量就会变成内部函数的局部变量，不再是外部函数的嵌套作用域中的变量，此时上面代码中的变量也变成局部变量，但是在定义前调用，会报错
    print(a)
    bar()
